	...
	switch(eventType) {
		...
		case CAN_EVENT_RX_COMPLETE:
		{
			if(objIdx == RX_MB_DATA) /* Received CAN DATA frame */
			{
				...
				{
					case ENGINE_RPM: {
						memcpy(&cluster.engine_rpm, user_rxBuff.data, sizeof(cluster.engine_rpm));
						break;
					}

					case ENGINE_TEMP: {
						memcpy(&cluster.engine_temp, user_rxBuff.data, sizeof(cluster.engine_temp));
						break;
					}

					case IGNITION_SWITCH: {
						memcpy(&cluster.ignition, user_rxBuff.data, sizeof(cluster.ignition));
						break;
					}

					case HANDBRAKE : {
						memcpy(&cluster.handbrake, user_rxBuff.data, sizeof(cluster.handbrake));
						break;
					}
